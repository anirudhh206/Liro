...
import { getSwapQuote, buildSwapTransaction, swapTransactionToHex } from "../services/swap/jupiter.service.js";
...
const investBodySchema = z.object({
  asset: z.string().refine(isAssetSymbol, "asset must be one of the fixed basket symbols"),
  amountUsd: z.string().regex(/^\d+(\.\d{1,2})?$/, "amountUsd must be a decimal string"),
});

const USDC_DECIMALS = 6;

export default function investRoutes(fastify: FastifyInstance): void {
  fastify.post("/invest", { preHandler: [fastify.authenticate, fastify.idempotent] }, async (request, reply) => {
    ...
    const amountLamports = new Decimal(body.amountUsd).mul(new Decimal(10).pow(USDC_DECIMALS)).toFixed(0);
